#!/usr/bin/env python3
"""
AIOS Production Alert System — Azure Monitor Edition
- Polls Azure Monitor ContainerLogV2 every 30s (with ingestion lag)
...
AZURE_TOKEN     = C.GRAFANA_TOKEN
AZURE_DS_UID    = C.AZURE_DS_UID
AZURE_RESOURCE  = C.AZURE_RESOURCE
NAMESPACE       = C.NAMESPACE
APP_CONTAINERS  = C.APP_CONTAINERS
...
INGEST_LAG_SEC  = C.INGEST_LAG_SEC
UI_PORT         = C.UI_PORT
...
# ── Azure Monitor ─────────────────────────────────────────────────────────────
def query_azure_monitor(start_iso: str, end_iso: str) -> list:
    """Query ContainerLogV2 for errors. Returns (ts_iso, container, pod, log_message)."""
    global _last_poll_time
    containers_filter = ",".join(f'"{c}"' for c in APP_CONTAINERS)
    kql_start = kql_ts(start_iso)
    kql_end   = kql_ts(end_iso)
    kql = f"""ContainerLogV2
| where PodNamespace == "{NAMESPACE}"
| where ContainerName in ({containers_filter})
| where TimeGenerated > datetime({kql_start}) and TimeGenerated <= datetime({kql_end})
| where LogLevel in ("error","critical","fatal")
    or (isempty(LogLevel) and LogMessage has_any ("ERROR","CRITICAL","FATAL","Traceback","Exception:"))
...
| take 200"""
    ...
    try:
        r = requests.post(
            ...
            json={
                "queries": [{
                    "refId": "A",
                    "datasource": {"type": "grafana-azure-monitor-datasource", "uid": AZURE_DS_UID},
        ...
        log(f"ERROR Azure Monitor query failed: {e}")
        return []

    results = r.json().get("results", {}).get("A", {})
    if results.get("error"):
        log(f"ERROR Azure Monitor: {results['error']}")
        return []
    ...
    for frame in frames:
        ...
        if not fields or not values:
            continue
        col = [f.get("name") for f in fields]
        try:
            ti = col.index("TimeGenerated"); ci = col.index("ContainerName")
            pi = col.index("PodName");       mi = col.index("LogMessage")
        except ValueError:
            continue
        ...
        for i in range(n):
            ...
            if isinstance(ts_val, (int, float)):
                ts_iso = datetime.fromtimestamp(ts_val / 1000, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f") + "Z"
            else:
                ts_iso = str(ts_val)
            entries.append((ts_iso, values[ci][i] or "", values[pi][i] or "", values[mi][i] or ""))

    _last_poll_time = datetime.now(timezone.utc).strftime("%H:%M:%S UTC")
    log(f"POLL {start_iso[11:19]} → {end_iso[11:19]} | {len(entries)} entries")
    return entries
...
<h1>AIOS Alert Manager <span class="source-badge">Azure Monitor</span></h1>
...
def main():
    setup()
    rotate_log()
    log(f"Started | Source=Azure Monitor | poll={POLL_SECONDS}s | lag={INGEST_LAG_SEC}s | UI=:{UI_PORT}")
    ...
    while True:
        ...
        # Query up to INGEST_LAG_SEC ago so Azure has time to ingest the logs
        end_ts  = (datetime.now(timezone.utc) - timedelta(seconds=INGEST_LAG_SEC)).strftime("%Y-%m-%dT%H:%M:%S.%f") + "Z"
        entries = query_azure_monitor(last_ts, end_ts)
